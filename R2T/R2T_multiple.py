...
def r2t_run(ipath, eps, b, gsensitivity, n_runs):
    global real_query_result
    global input_file_path
    global epsilon
    global beta
    ...
    input_file_path = ipath
    epsilon = eps
    beta = b
    global_sensitivity = gsensitivity
    runs = n_runs
    ...
    for i in range(runs):
        ...
        errors.append(abs(real_query_result-res)/real_query_result)
    return real_query_result, np.mean(results), np.std(results), np.mean(errors), np.std(errors), np.mean(times), np.std(times)
...
def main(argv):
    ...
    global real_query_result
    try:
        opts, args = getopt.getopt(argv,"h:I:e:b:G:p:r",["Input=","epsilon=","beta=","GlobalSensitivity=","ProcessorNum=","Runs="])
    except getopt.GetoptError:
        print("R2T.py -I <input file> -e <epsilon(default 0.1)> -b <beta(default 0.1)> -G <global sensitivity(default 1000,000)> -p <processor number(default 10)> -r <runs(default 1)>")
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
    ...
    if processor_num <1:
        processor_num = 1
    # Repeat for runs times and report avg and std dev of the results
    results = []
    errors = []
    times = []
    final_results = {}
    ...
    for eps in epsilon_list:
        ...
        for i in range(runs):
            ...
            results.append(res)
            times.append(end-start)
            errors.append(abs(res-real_query_result)/real_query_result)
        print("Query Result")
        ...
        final_results[eps] = {'result': np.mean(results), 'error': np.mean(errors), 'time': np.mean(times), 'std': np.std(results), 'std_error': np.std(errors), 'std_time': np.std(times)}
    
    n_nodes = 15000
    database = 'BR2000'
    method = 'positive_degree_nodes'
    import os
    import pickle
    
    results_folder = f'../../Plots/Results/{database}/{int(n_nodes)}'
    if not os.path.exists(results_folder):
        os.makedirs(results_folder)
    pickle.dump(final_results, open(f'{results_folder}/r2t_{method}.pkl', 'wb'))
